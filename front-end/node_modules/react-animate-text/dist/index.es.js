import { Component, createElement, createContext, Children, isValidElement, cloneElement, Fragment } from 'react';
...
var getSource$ = function (charInterval, limit) {
    return interval(charInterval).pipe(map(function (val) { return val + 1; }), take(limit));
};
...
var TextWrapper = (function (_a) {
    var text = _a.text;
    return (createElement(Consumer, null, function (_a) {
        var caret = _a.caret, animation = _a.animation;
        return (createElement(Fragment, null,
            animation === 'delete' && (caret || null),
            createElement(Text, null, String(text)),
            animation !== 'delete' && (caret || null)));
    }));
});
    ...
    var wrapChildren = function (children) {
        ...
        var wrappedChildren = childrenArray.map(function (child) {
            if (isValidElement(child) && child.props.children) {
                return cloneElement(child, __assign({}, child.props, { children: wrapChildren(child.props.children) }));
            }
            if (['string', 'number'].includes(typeof child)) {
                var textPosition = textCounter - wrappedTextIndex;
                textCounter++;
                if (textPosition === 0) {
                    return (createElement(TextWrapper, { key: textCounter, text: child }));
                }
...
var TextAnimation = (function (_super) {
    ...
    TextAnimation.prototype.render = function () {
        ...
        return (createElement(Provider, { value: {
